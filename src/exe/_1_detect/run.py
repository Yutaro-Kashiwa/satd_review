...
def run(project, start, stop, workers=10):
    ...
    df = mark_satd(df)
    # output
    df_file_dir = f"{ENV['out_dir']}/{project['name']}/{start}-{stop}"
    os.makedirs(df_file_dir, exist_ok=True)
    df.to_pickle(df_file_dir + "/df.pkl")
    # err
    err_file_dir = f"{ENV['out_dir']}/{project['name']}/{start}-{stop}"
    os.makedirs(err_file_dir, exist_ok=True)

    with open(err_file_dir + "/error.csv", mode='w') as f:
        for e in error:
            val = error[e]
...
if __name__ == '__main__':
    ...
    if len(args) == 5:
        ...
        stop = int(args[3])
        workers = int(args[4])
        run(project, start, stop, workers)
    else:
        run(PROJECT, 0, int(PROJECT['last_review_no']), workers=10)
