...
bot_list = ['Jenkins', 'Zuul', 'Qt Sanity Bot', 'Qt CI Bot'] #+後ろにCIがつくもの
...
if ignore >= 1: #途中から始める時の処理
    path = './csv/result_RQ2-2_'+ sub_name +'.csv'
    with open(path, 'r') as h:
...
for number in json_dic["not_have"]:
    list_count += 1
    ...
    print "now:" + str(list_count) +"/"+ str(len(json_dic["not_have"]))
    #先ずは全パッチ情報をゲットせよ．
    #パッチ総数をどうやってとるか？
    patch = 1
    ...
    temp_line = "not_have" + "," + str(number) + "," + status + "," + str(total_patch) + "," + str(diff_size) + "," + str(td) + "\n"
    output = temp_line
    path = './data/result_RQ2-2_' + sub_name + '.csv'
